...
import {Text, ScrollView, StyleSheet, View,KeyboardAvoidingView, ActivityIndicator, Alert} from 'react-native';
...
export default class Formio extends React.Component {
  static getDerivedStateFromProps(props, state) {
    ...
    if (props.form.components.length !== state.form.components.length) {
      submission.data = props.form.components.reduce(function (acc, comp) {
        if (comp.defaultValue) {
          acc[comp.key] = comp.defaultValue;
        }
        return acc;
      }, {});
    }
  ...
  // onEvent(event) {
  //   const newData = Object.assign(this.state.submission.data, this.data);
  //   if (typeof this.props.onEvent === 'function') {
  //     this.props.onEvent.apply(null, [event, newData, ...Array.prototype.slice.call(arguments, 1, arguments.length)]);
  //   }
  // }

  onEvent(event) {
    this.setPristine(false);
    console.log('on-event', this.state.isValid);
    if (!this.state.isValid) {
      Alert.alert('Alert', 'Please enter all the required fields to proceed', [
        {
          text: 'OK',
        },
      ]);
  ...
  validate(next) {
    ...
    Object.keys(inputs).forEach((name) => {
      // console.warn('all-valid', !inputs[name].state.value.isValid, name);
      if (inputs[name].state && !inputs[name].state.isValid) {
        allIsValid = false;
      }
    });
  ...
  onSubmit(event) {
    ...
    this.setPristine(false);
    console.log('on-submission', this.state.isValid);
    if (!this.state.isValid) {
      Alert.alert('Alert', 'Please enter all the required fields to proceed', [
        {
          text: 'OK',
      ...
      // this.showAlert('danger', 'Please fix the following errors before submitting.', false);
      // if (this.props.onFormError) {
      //   this.props.onFormError({
    ...
    if (
      sub.hasOwnProperty("data") &&
      typeof sub.data === "object" &&
      Object.keys(sub.data).length > 0
    ) {
      sub.data = Object.assign(sub.data, clone(this.data));
    } else {
      sub.data = clone(this.data);
    }
    sub.state = 'submitted';
